...
export default function GridScan({
  ...
  const lookVelocity = useRef(new THREE.Vector2(0, 0));
  ...
  useEffect(() => {
    ...
    const pushScan = (t: number) => {
      const next = scanStartsRef.current.slice();
      if (next.length >= MAX_SCANS) next.shift();
      next.push(t);
      scanStartsRef.current = next;
      const material = materialRef.current;
      if (!material) return;
      const buffer = new Array(MAX_SCANS).fill(0);
      for (let i = 0; i < next.length; i += 1) buffer[i] = next[i];
      material.uniforms.uScanStarts.value = buffer;
    ...
    const uniforms = {
      ...
      uLineJitter: { value: THREE.MathUtils.clamp(lineJitter, 0, 1) },
    ...
    const resize = () => {
      ...
      composerRef.current?.setSize(element.clientWidth, element.clientHeight);
    };
    ...
    if (scanOnClick) element.addEventListener("click", handleClick);

    const smoothTime = THREE.MathUtils.lerp(0.45, 0.12, THREE.MathUtils.clamp(sensitivity, 0, 1));
    ...
    const tick = () => {
      ...
      material.uniforms.uSkew.value.set(
        ...
        -lookCurrent.current.y * skewScale * 1.5,
      );
    ...
    return () => {
      ...
      if (scanOnClick) element.removeEventListener("click", handleClick);
